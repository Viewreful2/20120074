...
for t in range(0,T):#T만큼 빙고 게임을 하자
    ...
    input_numbers = [int(j) for j in input_numbers]#그 숫자들을 숫자로 만들어주자

    count_of_input = 0#몇번째 불리는 숫자인지를 알기 위한 변수
    for number in input_numbers:#입력 받은 숫자들에 대해서 시작

        count_of_input = count_of_input + 1# 몇번째 불리는지 알기 위해 더해주자.

        for i in range(0,5):
            for j in range(0,5):
                if number == BINGO[i][j]:
                    B[i][j] = 1
                    #존재한다면 B에서 그 해당 위치의 값을 1로 바꿔준다.
        if a1(B) == True:
